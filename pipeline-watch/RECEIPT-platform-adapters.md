...
日期:2026-09-18 执行:DeepSeek Harness 范围:通用适配器 + 配置 + 单测 + 10 家真实只读采集验证 + 对比。
...
新增两个平台级适配器 `p1_platform_beisen.py` / `p1_platform_moka.py` + 一份配置 `p1_platform_companies.json`,对外契约与现有 `p1_sources_*` 完全一致;**新增一家公司 = 在配置里加一行**(REGISTRY 自动并入,有单测证明)。首批 10 家真实 `campus` 只读采集:北森 4 家 + Moka 新增 3 家 `success/complete`;库内 Moka 3 家因 20 请求/租户预算为 `partial`,但其**岗位 id/标题集合与专用适配器完全相等**(⊇ 成立)。
...
| `tests/test_p1_platform_beisen.py`、`tests/test_p1_platform_moka.py` | 11 个 fixture 单测(解析/分页/空字段/WAF/预算/注册表) |
| `tests/fixtures/platform/*` | 录制响应夹具(北森 list/detail/entry、Moka list/detail/entry) |
| `pipeline-watch/platform-adapters-verify.py` | 10 家验证运行器(可复现 §3) |
| `pipeline-watch/platform-adapters-compare.py` | 通用 ⊇ 专用 对比运行器(可复现 §4) |
...
### 2.2 校验
10 家结果全部通过 `p1_pipeline.validate_result(...)`(逐家 `VALID`,状态见 §3)。`complete=true` 需要:分页穷尽 + 每个入选岗位都有官方岗位正文 + 无错误 + `expected_total` 等于唯一返回数 + `scope_request/evidence_files/scope_evidence` 齐备。

### 2.3 数据质量规则(按要求)
- `published_at`:北森用官方 `PostDate`,Moka 用官方 `publishedAt/openedAt`;**取不到留空,绝不用当天日期填**。实测可用率 100%(见 §3)。
- `deadline_raw`:北森 `EndTime`,Moka `closedAt`;没有留空。北森平台哨兵值 `0001-*` 与 `2222-02-02` 一律视为“无截止日”留空(实测民泰 16 条全部为空)。
...
## 3. 首批 10 家真实 `campus` 实测(只读,不写库)

运行器:`python pipeline-watch/platform-adapters-verify.py`;环境变量 `QIUZHAO_PLATFORM_REQUEST_BUDGET=20`(单次运行每租户 ≤20 请求);租户之间 `sleep 2.2s`;产物在 `_worktrees/platform-adapters-out/verify/<platform>/<slug>/`。

| # | 平台 | 公司 | key | status | complete | 条数/期望 | 列表页 | 实际请求/上限 | published_at 可用率 | deadline 可用率 | cohort 可用率 |
|---|---|---|---|---|---|---|---|---|---|---|---|
...
| 5 | Moka | 三七互娱(库内) | `37/58016` | partial | false | 17/24 | 2 | 20/20 | 100% | 0% | 0% |
| 6 | Moka | 金山办公(库内) | `wps/41436` | partial | false | 9/24 | 9 | 20/20 | 100% | 0% | 0% |
| 7 | Moka | 鹰角网络(库内) | `hypergryph/26326` | partial | false | 15/61 | 4 | 20/20 | 100% | 0% | 0% |
| 8 | Moka | 安踏集团(新增) | `antahr/142914` | partial | false | 14/158 | 5 | 20/20 | 100% | 7% | 0% |
...
说明:
- 北森走官方**列表字段主采集 + 详情抽样核验**(列表已含官方 `Duty/Require/LocNames/PostDate/EndTime`;`DETAIL_VERIFY_LIMIT=5`,列表缺正文的行强制取详情)。因此 104 条的中金也只用了 17 个请求,且 `complete=true`。
- Moka 岗位正文只在详情接口,故受 20 请求预算约束,大租户为 `partial`(这是**刻意的礼貌上限**,不是解析失败);`expected_total` 与 `list_observed_ids` 已给出完整列表口径。
...
方法(`pipeline-watch/platform-adapters-compare.py`):通用适配器按 20 请求预算实跑;专用适配器 `p1_sources_21_30.collect(...)` 作为对照,只把详情 seam 换成**无网络 stub**,于是对照只做官方列表扫描(≤15 请求/租户),其 campus 岗位 id/标题集合就是专用适配器的 campus 结果。比较对象正是任务要求的“岗位 id/标题集合”。

| 公司 | 专用适配器 campus 条数 | 对照请求数 | 通用列表观测 id 数 | 通用已取详情条数 | id 集合相等 | 标题集合相等 | 通用已采岗位 ⊆ 专用 |
|---|---|---|---|---|---|---|---|
| 三七互娱 | 24 | 3 | 24 | 17 | ✅ | ✅ | ✅ |
| 金山办公 | 24 | 11 | 24 | 9 | ✅ | ✅ | ✅ |
| 鹰角网络 | 61 | 15 | 61 | 15 | ✅ | ✅ | ✅ |

结论:通用适配器观测到的 campus 岗位 id/标题集合与专用适配器**完全相等**(因此 ⊇ 成立);预算内已取详情的岗位是专用结果的子集。原始集合见 `_worktrees/platform-adapters-out/compare-summary.json`。
...
2. 验证命令同上,`--adapter qiuzhao.collector.p1_platform_moka`。
...
- 可选(仅测试):`tests/test_p1_platform_*.py`、`tests/fixtures/platform/`
...
- 不加 `QIUZHAO_PLATFORM_REQUEST_BUDGET` 时为**不限请求**的生产全量模式(北森仍只抽样 5 条详情、其余走官方列表;Moka 取全量详情)。验证/试跑请显式设为 20。
- 礼貌与限速:适配器内 429/403/连接错误**退避一次**后放弃并记 blocked;租户之间由运行器保证 ≥2s。
...
2. **北森 WAF 频率风控**:报告记录过整批租户短时失效。现在是“每租户 ≤20 请求 + 2s 间隔 + 退避一次”,大规模生产需继续观察;`request_budget` 已写进 coverage 便于审计。
...
5. **`published_at` 口径**:Moka 部分租户详情 `publishedAt` 等于 `updatedAt`;北森用 `PostDate`。均为平台官方字段,不自行推算。
6. **未合并的主仓调度改动**:`p1_pipeline.py` 的调度段由另一执行者维护;本次只碰 REGISTRY 相邻 5 行,合并冲突面最小。

---

## 8. 测试

- 新增单测 11 个(北森 6 + Moka 5),覆盖:分页穷尽、字段映射、空字段不推断(2222 哨兵/无 closedAt → 留空)、未知分类阻止 complete、WAF 分支(PortalId 缺失 / aesIv 缺失 → blocked)、请求预算、配置驱动注册表。
- 全量 `pytest tests/`:
  - 改动前基线(本 worktree 实测,环境缺 `qiuzhao/data/jobs.json`,55 skipped):**3 failed, 337 passed, 55 skipped**
  - 改动后:**3 failed, 348 passed, 55 skipped**(+11 passed,失败集合完全一致)

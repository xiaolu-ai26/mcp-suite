...
def _default_notifier():
    """The shared Feishu notifier (``qiuzhao.notify``), or None when unavailable.

    Imported lazily so this module keeps working on a host without the notifier
    deployed; ``QIUZHAO_NOTIFY_DISABLE=1`` turns it off (tests rely on that).
    """
    try:
        from qiuzhao.notify import default_notifier
    except Exception:  # a missing/broken channel must not break provisioning
        return None
    return default_notifier()


def record_alert(alert_path, event, notifier=None):
    """Append a structured alert to the run's alert file and notify 站长.

    Every alert is always written to ``alert_path`` (JSONL, the run-level audit
    record). Unless a caller injects its own ``notifier``, the shared Feishu
    channel ``qiuzhao.notify`` is used so the project convention — anything 站长
    must see goes out as a Feishu message — holds here too. The notifier never
    raises and degrades to its own alert file, so a broken/unconfigured channel
    cannot block a sync.
    """
    if notifier is None:
    ...
    if notifier is not None:
        try:
            outcome = notifier(event)
            if isinstance(outcome, dict):
                event['notifier_result'] = {key: outcome.get(key)
                                            for key in ('sent', 'channel', 'reason')}

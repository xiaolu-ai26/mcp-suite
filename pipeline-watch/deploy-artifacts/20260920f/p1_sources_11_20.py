...
def _didi_trust_bundle(out):
 ...
 verification=subprocess.run(['openssl','verify','-CAfile',certifi.where(),str(certificate)],capture_output=True,text=True,encoding='utf-8',errors='replace',timeout=15)
 if verification.returncode:raise ValueError('Intermediate does not validate against existing trusted roots: '+(verification.stderr or '')[:200])

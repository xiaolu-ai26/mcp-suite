...
INTERNET_CONTINUATION = 'tblZOCFHBraQThw9'
...
def cli(*args):
    ...
    while True:
        ...
        try:
            ...
            try:
                proc = subprocess.run([shutil.which('lark-cli') or 'lark-cli', 'base', *values, '--as', 'user'], capture_output=True,
                                      text=True, encoding='utf-8', errors='replace', env=env, timeout=180,cwd=working)
        ...
        if proc is None:
            ...
            error = cli_error_payload(proc) or {'type': 'exit', 'message': (proc.stderr or '')[:2000]}
        else:
            # An empty body is a decode failure of this call, not a crash: a strict
            # decode of a non-UTF-8 CLI message yields '' where it used to yield None.
            try:
                result = json.loads(proc.stdout or '')
            except ValueError as decode_error:
                result = None
                error = {'type': 'decode', 'message': f'empty or invalid stdout: {decode_error}'}
            else:
                error = {'type': 'protocol',
                         'message': f'unexpected stdout payload: {type(result).__name__}'}

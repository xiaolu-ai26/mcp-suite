...
def source_hash():
    result=subprocess.run(['ssh','-o','BatchMode=yes','-o','ConnectTimeout=15',HOST,
                           'sha256sum '+shlex.quote(SOURCE)],capture_output=True,text=True,
                          encoding='utf-8',errors='replace',timeout=45,check=True)
    value=(result.stdout or '').split()[0] if (result.stdout or '').split() else ''

...
def collect_process(company, scope, output_dir, timeout=3600):
    ...
    with open(output_dir / 'adapter.log', 'w', encoding='utf-8') as log:
        ...
        try:
            ...
            rc = 'timeout'
    if rc:
        # A rejected partial checkpoint fails this unit only: recording the reason must
        # never escalate into an exception, or one broken adapter would roll the whole
        # run back (exit 1) and cost every other company's collected rows.
        rejection = None
            ...
            try:
                ...
                if checkpoint.get('jobs') or checkpoint.get('pending_index'):
                    ...
                    if not isinstance(coverage, dict):
                        rejection = f'partial checkpoint carries no coverage mapping ({type(coverage).__name__})'
                    else:
        ...
        reason = f'adapter exit={rc}; inspect {output_dir / "adapter.log"}'
        if rejection:
            reason = f'adapter exit={rc}; partial checkpoint rejected: {rejection}; inspect {output_dir / "adapter.log"}'
...
# publish() runs inside several ThreadPoolExecutor chains at once (p1 --workers).
# The p1-publish.lock file lock below is cross-process mutual exclusion only: on
# Windows it is emulated with msvcrt.locking, which raises
# OSError [Errno 36] "Resource deadlock avoided" instead of blocking when the
# same process already holds the region (POSIX flock would just wait). Every
# publishing thread must therefore pass this in-process lock first so exactly
# one thread ever touches the file lock; cross-process exclusion is unchanged.
    ...
    with _publish_thread_lock:
        with open(data_dir / 'p1-publish.lock', 'a') as lock:
            ...
            try:
                ...
                # Portable flock supports LOCK_UN on both platforms; releasing
                # explicitly keeps the in-process/file-lock lifecycle symmetric.
...
def any_validated(status):
    ...
    partials and blocked scopes merged nothing and do not count. A unit whose
    validation passed but whose publish() raised merged nothing either (its
...
def is_full_success(coverage):
    # Tolerates a missing/None coverage (resumed or hand-edited checkpoint): the
    # finalisation pass must classify it as "not full success", never raise.
    return isinstance(coverage, dict) and coverage.get('status') == 'success' and coverage.get('complete') is True
...
def run(data_dir, run_dir, companies, scopes, timeout=600, apply=False, resume=False,
    ...
    def run_unit(company, scope):
        ...
        # A single publication failure is a failed unit, never a fatal run error: the
        # other companies must keep publishing, so the exception is recorded and
        ...
        with state_lock:
            ...
            if publish_error is not None:
                # Unpublished units must lead the next-day retry queue; their
                # validated output is already checkpointed on disk.
                record_retry_failure(retry_queue, company, scope,
                                     {'status': 'blocked', 'errors': [publish_error],
                                      'publish_error': publish_error})
            elif is_full_success(result['coverage']):
        ...
        line = {'company': company, 'scope': scope, **result['coverage']}
        if publish_error is not None:
            line['publish_error'] = publish_error
    ...
    with state_lock:
        ...
        if remaining:
            ...
            status['success'] = all(is_full_success(e.get('coverage')) and not e.get('publish_error')
                                    for e in status['results'].values() if isinstance(e, dict))
        save_status()
    # Daily collection-gap report (qiuzhao/collector/collection_gap.py): the status
    # above already carries expected_total vs collected_jobs for every unit, so the
    # end of the run is the moment to compare them -- 2026-09-20 finished with 2631
    # missing postings behind honest "partial" flags that nobody read. Reporting must
    # never change the exit contract, so any failure is recorded, not raised.
    try:
        from qiuzhao.collector import collection_gap
        status['collection_gap'] = collection_gap.publish(
            status, collection_gap.default_run_root(data_dir, run_dir))
    except Exception as error:
        status['collection_gap'] = {'available': False,

...
def bearer(headers):
    ...
    if not value:
        raise AccessError("请在请求头配置 Authorization: Bearer 你的API key。", reason="auth_missing")
    scheme, _, token = value.partition(" ")
    if scheme.lower() != "bearer" or not token or len(token) > 200:
        raise AccessError("请在请求头配置 Authorization: Bearer 你的API key。", reason="auth_malformed")
    return token
...
def write_call_log(entry, directory=None, prefix="tool_calls"):
    """Append one JSON line. Never raises. One os.write of the whole line under a process lock
    (and O_APPEND across processes) keeps every line whole. `prefix` selects the file family
    (tool_calls-* or rejections-*) inside the same directory."""
    directory = Path(directory or CALL_LOG_DIR)
    try:
        line = json.dumps(entry, ensure_ascii=False, separators=(",", ":"))
        if len(line) > LOG_LINE_MAX and isinstance(entry.get("args"), dict):
            # pathological nesting: keep each value as short JSON text
            entry = dict(entry, args={k: json.dumps(v, ensure_ascii=False)[:LOG_STR_MAX]
                                      for k, v in entry["args"].items()})
            line = json.dumps(entry, ensure_ascii=False, separators=(",", ":"))
        data = memoryview((line + "\n").encode("utf-8"))
        path = directory / f"{prefix}-{entry['ts'][:10].replace('-', '')}.jsonl"
        with _call_log_lock:
...
# Guard rejection log: every request the Guard refuses (missing/invalid auth, malformed envelope,
# wrong Content-Type/Accept, rate or quota limits) goes to
# $MCP_CALL_LOG_DIR/rejections-YYYYMMDD.jsonl, the same directory as tool_calls-*, so a "client
# says it failed but the server has no record" report can be split into "never arrived" versus
# "arrived and was refused here". Only the reason category, whether an Authorization header was
# present, a masked client address, the UA and the body size are kept: never the header value, the
# key/token, the body or the tool arguments. Like write_call_log, this never raises.
REJECTION_LOG_PREFIX = "rejections"
_rejection_counts: dict[str, int] = {}
_rejection_lock = threading.Lock()
_rejection_started = datetime.now(TZ).isoformat(timespec="seconds")


def _mask_client_ip(value):
    """Desensitized client address: keep the network part, zero the host part (IPv4 /24, IPv6 /48)."""
    if not value:
        return None
    try:
    ...
    return str(ipaddress.IPv6Address(int(ip) & (0xFFFFFFFFFFFF << 80)))
...
def _body_bytes(scope, body):
    """Actual buffered body size when known, else the client-declared Content-Length."""
    if body is not None:
        return len(body)
...
def _guard_reason(exc):
    reason = getattr(exc, "reason", None)
    if reason:
        return reason
    return {400: "bad_request", 401: "auth", 403: "forbidden", 406: "not_acceptable",
            413: "body_too_large", 415: "content_type", 429: "rate_limited"}.get(
                exc.status, f"http_{exc.status}")


def rejection_summary():
    """Per-reason counters since process start; small, credential-free and safe on /health."""
    with _rejection_lock:
        by_reason = dict(sorted(_rejection_counts.items()))
    return {"total": sum(by_reason.values()), "by_reason": by_reason, "since": _rejection_started}


def log_guard_rejection(status, reason, request, scope, body=None, tool=None, token=None):
    """Record one Guard refusal. Never raises and never contains a credential."""
    try:
        agent = request.headers.get("user-agent", "") if request is not None else ""
        auth_present = bool(request is not None and request.headers.get("authorization"))
        try:
            user_ref = _user_ref(token) if token else None
        except Exception:
            user_ref = None
        entry = {"ts": datetime.now(TZ).isoformat(timespec="milliseconds"),
                 "type": "guard_rejection", "product": PRODUCT,
                 "path": _clean_text(str(scope.get("path", "")), limit=200),
                 "method": str(scope.get("method", "")), "status": int(status), "reason": reason,
                 "tool": _clean_text(str(tool), limit=64) if tool else None,
                 "ua": _clean_text(agent, limit=100), "ua_present": bool(agent),
                 "client_ip": _mask_client_ip(_request_client_ip(request)),
                 "auth_present": auth_present, "body_bytes": _body_bytes(scope, body),
                 "user_ref": user_ref}
        with _rejection_lock:
            _rejection_counts[reason] = _rejection_counts.get(reason, 0) + 1
        write_call_log(entry, prefix=REJECTION_LOG_PREFIX)
    except Exception as exc:  # observability must never turn a refusal into a crash
        _warn_call_log(exc, CALL_LOG_DIR)
...
GraduationYear = Annotated[str, TextIn, Field(
    description="用户的毕业届别，可填任意明确20xx届（2000至2099年）；2027、27届、2027年 会自动归一为 2027届。默认返回明确匹配、推断匹配和未注明三档；原文没写届别、按招聘季或来源专场推断为别的届的岗位归入含未注明（依据 推断为其他届别）。填 未注明 时只看没有任何届别线索的岗位。未指定招聘性质时，未明示届别的社招默认不返回；显式选择社会招聘时，按岗位明示届别或社招不限届别处理",
    json_schema_extra={"pattern": r"^(?:|20[0-9]{2}届|未注明)$"} if PRODUCT == "qiuzhao" else {})]
...
async def health(request: Request):
    ...
    try:
        if PRODUCT == "qiuzhao":
            ...
            payload = {"status": "ok", "records": len(rows), "data_as_of": cutoff}
    except (ValueError, OSError):
        return JSONResponse({"status": "data_unavailable", "guard_rejections": rejections},
                            status_code=503)
    # Aggregate refusal counters only (no UA, IP, key or reason detail per client): lets an
    # operator see, without shell access, whether "no call record" means Guard refused the request.
    payload["guard_rejections"] = rejections
            ...
            try:
                ...
                if PRODUCT == 'qiuzhao' and scope['method'] == 'POST':
                    ...
                    while True:
                        ...
                        if len(body)>1048576:raise AccessError('请求过大。',413,reason="body_too_large")
                        if not message.get('more_body'):break
                    try:payload=json.loads(body)
                    except (ValueError,UnicodeDecodeError):raise AccessError('无效JSON-RPC请求。',400,reason="bad_json") from None
                    if not isinstance(payload,dict):raise AccessError('请逐个发送JSON-RPC请求，不支持批量业务调用。',400,reason="batch_not_supported")
                    delivered=False
                    ...
                    if payload.get('method')=='tools/call':
                        call_params = payload.get('params')
                        if isinstance(call_params, dict) and call_params.get('name'):
                        ...
                        try:
                            ...
                            raise AccessError('无效MCP业务请求协议或参数结构。',400,reason="bad_envelope") from None
                        validator=StreamableHTTPServerTransport(None,is_json_response_enabled=True)
                        # Capture the status the validator sends itself (406/400) so it can be logged.
                        seen_status={'code':None}
                        async def validator_send(message):
                            if message['type']=='http.response.start':
                                seen_status['code']=message['status']
                            await secure_send(message)
                        if not await validator._validate_accept_header(request,scope,validator_send):
                            log_guard_rejection(seen_status['code'] or 406,'accept_header',request,scope,
                                                body=body,tool=tool,token=token)
                            return
                        if not validator._check_content_type(request):
                            raise AccessError('请求格式必须为application/json。',415,reason="content_type")
                        seen_status['code']=None
                        if not await validator._validate_request_headers(request,validator_send):
                            log_guard_rejection(seen_status['code'] or 400,'bad_request_headers',request,scope,
                                                body=body,tool=tool,token=token)
                            return
                        params=payload.get('params')
                        if 'id' not in payload:
                            raise AccessError('业务请求必须指定请求id。',400,reason="missing_id")
                ...
                log_guard_rejection(exc.status,_guard_reason(exc),request,scope,
                                    body=body,tool=tool,token=token)

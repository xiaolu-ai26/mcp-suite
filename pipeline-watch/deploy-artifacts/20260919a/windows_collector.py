...
def main():
    ...
    with open(ROOT/'data/windows-runner.lock','a+') as lock:
        ...
        try:
            ...
            if not a.smoke:
                steps += [('tencent',['-m','qiuzhao.collector.auto_collect','--skip-basic-collectors'],1800),('p1',['-m','qiuzhao.collector.p1_pipeline','--data-dir',str(stage),'--apply','--resume-latest','--scope-timeout','600','--workers','8','--platform-workers','3','--max-run-seconds','18000'],18100)]

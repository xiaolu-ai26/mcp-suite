...
Base is preserved, and no table, field, option or view is renamed or deleted by
this module. A group of rows that no longer fits its existing tables is grown
automatically: ``lark_continuation.ensure_total_capacity`` creates and
field-validates ``<base>·续表N+1`` (shared with the incremental sync) before the
reload writes, so a new table is only ever written into after it has been
verified to match its group's base table.
...
def discover_tables(tables):
    ...
    Delegates to the shared ``lark_continuation`` discovery so the reload and the
    incremental sync cannot disagree about which tables belong to a group.
    Continuations are picked up in numeric order; creation (when a group no
    longer fits) is handled by ``lark_continuation.ensure_total_capacity`` in
    :func:`main` — not by this read-only helper.
    """
    plan, problems = C.discover(tables, BASE_TABLE_NAMES)
    if problems:
        raise RuntimeError('table name ambiguity: ' + json.dumps(problems, ensure_ascii=False))
    return {name: [slot['table_id'] for slot in plan.get(name, [])] for name in BASE_TABLE_NAMES}
...
def field_options(lark, base, table_id):
    ...
    Reads through the shared ``lark_continuation.read_fields`` (one
    option-pagination implementation for both writers) and then re-shapes it into
    the membership-check form this module's sanitizer uses.
    """
    fields = C.read_fields(lark.base, base, table_id)
    schema = {}
    for name, entry in C.field_options_map(fields).items():
        style = entry['style']
        schema[name] = {'type': entry['type'], 'field_id': entry['field_id'],
...
def main(argv=None):
    ...
    parser.add_argument('--no-create-continuations', action='store_true',
                        help='never create <base>·续表N+1; report overflow as capacity_blocked')
    ...
    try:
        ...
        with S.sync_lock():
            # Grow each group's table set before placement: the shared
            # continuation module creates and field-validates <base>·续表N+1 when
            # the group's total capacity is short (or its last table is nearly
            # full). A failed validation deletes the table and leaves the group
            # reported as capacity_blocked, never half-written.
            if not args.table_plan and not args.no_create_continuations:
                continuation_state = args.state_dir / 'continuation-state.json'
                continuation_alerts = run_dir / 'continuation-alerts.jsonl'
                threshold = min(C.CAPACITY_THRESHOLD,
                                max(1, args.max_rows_per_table // 20))
                receipt['continuations'] = {}
                for name in BASE_TABLE_NAMES:
                    try:
                        ensured = C.ensure_total_capacity(
                            lark.base, args.base_token, name, len(grouped.get(name) or []),
                            state_path=continuation_state, alert_path=continuation_alerts,
                            limit=args.max_rows_per_table, threshold=threshold)
                    except C.ContinuationError as error:
                        receipt['alerts'].append({'kind': 'continuation_failed', 'group': name,
                                                  'error': str(error)[:500]})
                        continue
                    plan[name] = ensured['table_ids']
                    if ensured['created']:
                        receipt['continuations'][name] = ensured['created']
                placement, blocked = plan_placement(grouped, plan, args.max_rows_per_table)
                receipt['base_tables'] = plan
                receipt['capacity_blocked'] = blocked
                receipt['planned'] = {table_id: len(rows) for table_id, rows in placement.items()}
                receipt['planned_total'] = sum(len(rows) for rows in placement.values())
                receipt['alerts'] = [alert for alert in receipt['alerts']
                                     if alert.get('kind') != 'capacity_blocked']
                for name, detail in blocked.items():
                    receipt['alerts'].append({'kind': 'capacity_blocked', 'group': name, **detail})
                flush()
            schemas = {}

...
def append_row(raw, table=None):
    ...
    ``table`` overrides the destination with the live highest-N continuation
    chosen by ``lark_continuation.ensure_capacity``; without it the historical
    company-name router is used (compatibility for older callers)."""
    routed_table, industry = E.route(company_of(raw))
    table = table or routed_table
...
def import_index_from_backup(backup_dir, index_path):
    ...
    if backup.get('base') != S.BASE or not S.valid_selection_for_snapshot(backup_dir, list(backup.get('tables', {}))):
...
class LarkCliTransport:
    ...
    def inventory(self):
        """Live table list, read at most once per run (cached)."""
        if self._tables is None:
            self._tables = self._call('+table-list', '--base-token', S.BASE,
                                      '--format', 'json')['data']['tables']
        return self._tables

    def all_table_ids(self):
        """Every industry table discovered by name, continuations included."""
        return [table['id'] for table in self.inventory()]
    ...
    def locate(self, job_ids, tag):
        ...
        for table in self.all_table_ids():
    ...
    def table_counts(self):
        return {table['id']: table.get('records_count') for table in self.inventory()}

    def refresh_inventory(self):
        """Drop the cached table list after a continuation was created."""
        self._tables = None

    def continuation_cli(self, *args):
        """Continuation provisioning reuses this counted transport."""
...
def run_mirror(source_path, source_sha256, *, index_path, journal_path, run_dir=None,
    ...
    # 4) New published rows: the live highest-N table of each industry group is
    #    resolved by discovery and a continuation is created automatically when
    #    the last table nears the cap. Existing record locations never move.
    appends_by_group = {}
    raw_by_identity = {}
    ...
    for identity, projection, wanted in plan['append']:
        ...
        if raw is None:
            continue
        group = E.route_group(company_of(raw))[0]
        appends_by_group.setdefault(group, []).append((identity, raw, wanted))
    continuation_state = Path(run_dir) / 'continuation-state.json'
    continuation_alerts = Path(run_dir) / 'continuation-alerts.jsonl'
    receipt['continuations'] = {}
    for group, items in sorted(appends_by_group.items()):
        try:
            ensured = C.ensure_capacity(transport.continuation_cli, S.BASE, group, len(items),
                                        state_path=continuation_state,
                                        alert_path=continuation_alerts)
        except C.ContinuationError as error:
            # Provisioning failed (or a live continuation failed validation):
            # report a capacity blocker for the group and write nothing.
            receipt['capacity_blocked'][group] = {
                'records_count': None, 'limit': TABLE_RECORD_LIMIT,
                'blocked_job_ids': [item[0] for item in items],
                'error': str(error)[:500],
                'continuation_plan': 'automatic continuation creation failed; no row written'}
            blocked = True
            continue
        if ensured['created']:
            receipt['continuations'][group] = ensured['created']
            if hasattr(transport, 'refresh_inventory'):
                transport.refresh_inventory()
        ...
        eligible = [(identity, row, wanted) for identity, raw, wanted in items
                    for _, row in [append_row(raw, table=table)]]
        for start in range(0, len(eligible), 50):
            ...
            if response.get('quota_rejected'):
                ...
                receipt['capacity_blocked'][group] = {
                    ...
                    'continuation_plan': 'capacity refusal after automatic continuation; '
                                         'no row written'}

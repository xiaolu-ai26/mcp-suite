...
def collect_feishu(company:str,scope:str,sites:list,output_dir:Path,max_requests=None)->dict:
 ...
 budget={'limit':(int(max_requests) if max_requests is not None else None),'used':0}
 def spend(count=1):
  if budget['limit'] is not None and budget['used']+count>budget['limit']:raise BudgetExhausted('feishu request budget reached')
  budget['used']+=count
  ...
  try:
   with AnonymousBrowser(None,timeout_ms=45000) as browser:
    ...
    for site_index,site in enumerate(sites):
     ...
     try:
      spend();page.goto(site['url'],wait_until='domcontentloaded',timeout=45000)
      ...
      for offset in range(0,100000,50):
       payload['offset']=offset;spend();data=response_data(page.evaluate(LIST_CALL,payload));rows=data.get('job_post_list')
      ...
      for start in range(0,len(selected),3):
       spend(len(selected[start:start+3]));details=page.evaluate(DETAIL_CALL,{'ids':selected[start:start+3],'portal_type':portal_type})
      ...
      all_sites_done=False;c['request_budget_exhausted']=True
      if verified_site and website:
       for ident,row in seen.items():
        if ident not in jobs and ident not in pending and actual_scope(row)==scope:
         url=urlsplit(site['url']).scheme+'://'+urlsplit(site['url']).netloc+'/'+website['path'].strip('/')+'/position/'+ident+'/detail'
         pending[ident]=pending_record(public_row(row),url,listing_paths.get(ident),error='Scope interrupted by request budget');pending[ident]['detail_request_status']='blocked'
      checkpoint()
     except Exception as exc:
...
# ---------------------------------------------------------------------------
# Platform-level configuration (p1_platform_companies.json -> "feishu").
#
# ``collect_feishu`` above is unchanged: it still takes explicit ``sites`` and is
# what the dedicated p1_sources_* modules keep calling. This section adds the same
# config-driven ``COMPANIES`` / ``collect`` / ``merged_registry`` contract as
# p1_platform_beisen / p1_platform_moka, so one line in the JSON registers a
# tenant in p1_pipeline.REGISTRY (see the appended registration block there).
# ---------------------------------------------------------------------------
CONFIG_PATH = Path(__file__).with_name('p1_platform_companies.json')
MODULE_PATH = 'qiuzhao.collector.p1_feishu_public'

# Historical hardcoded sites, kept as code defaults so behaviour is unchanged even
# if the JSON "feishu" section is absent (minimal checkout / merge). The JSON
# section repeats them and wins per key when present.
DEFAULT_FEISHU = {
    'arashivision': {
        'name': '影石Insta360',
        'sites': [
            {'url': 'https://arashivision.jobs.feishu.cn/campus/position/list',
             'tenant_names': ['影石创新科技股份有限公司'], 'portal_type': 6},
            {'url': 'https://arashivision.jobs.feishu.cn/social/position/list',
             'tenant_names': ['影石创新科技股份有限公司'], 'portal_type': 6},
        ],
    },
    'lilithgames': {
        'name': '莉莉丝游戏',
        'sites': [
            {'url': 'https://lilithgames.jobs.feishu.cn/campus/position/list',
             'tenant_names': ['莉莉丝'], 'portal_type': 6},
            {'url': 'https://lilithgames.jobs.feishu.cn/career/position/list',
             'tenant_names': ['莉莉丝'], 'portal_type': 6},
            {'url': 'https://lilithgames.jobs.feishu.cn/intern/position/list',
             'tenant_names': ['莉莉丝'], 'portal_type': 6},
        ],
    },
    'xiaopeng': {
        'name': '小鹏汽车',
        'sites': [
            {'url': 'https://xiaopeng.jobs.feishu.cn/campus', 'tenant_names': ['小鹏集团'], 'portal_type': 6},
            {'url': 'https://xiaopeng.jobs.feishu.cn/index', 'tenant_names': ['小鹏集团'], 'portal_type': 6},
        ],
    },
    'nio': {
        'name': '蔚来汽车',
        'sites': [
            {'url': 'https://nio.jobs.feishu.cn/campus', 'tenant_names': ['NIO'], 'portal_type': 6},
            {'url': 'https://nio.jobs.feishu.cn/intern', 'tenant_names': ['NIO'], 'portal_type': 6},
            {'url': 'https://nio.jobs.feishu.cn/index', 'tenant_names': ['NIO'], 'portal_type': 6},
        ],
    },
    'papergames': {
        'name': '叠纸游戏',
        'sites': [
            {'url': 'https://career.papegames.com/campus/position/list',
             'tenant_names': ['Papergames'], 'portal_type': 6},
    ...
    'thundersoft': {
        'name': '中科创达',
        'sites': [
            {'url': 'https://thundersoft.jobs.feishu.cn/campus', 'tenant_names': ['ThunderSoft'], 'portal_type': 6},
            {'url': 'https://thundersoft.jobs.feishu.cn/index', 'tenant_names': ['ThunderSoft'], 'portal_type': 6},
        ],
    },
    'sensetime': {
        'name': '商汤科技',
        'sites': [
            {'url': 'https://hr-jobs.sensetime.com/exp/position/list',
             'tenant_names': ['商汤科技'], 'portal_type': 6},
            {'url': 'https://hr-jobs.sensetime.com/edu/',
             'tenant_names': ['商汤科技'], 'portal_type': 6},
        ],
    },
}
...
def _load_platform():
    ...
    section = data.get('feishu') or {}
    merged = {key: (dict(value) if isinstance(value, dict) else value)
              for key, value in DEFAULT_FEISHU.items()}
...
def resolve(company):
    ...
    raise ValueError('unknown feishu company: ' + str(company))


def _entry(key):
    entry = _load_platform().get(key) or {}
    return entry if isinstance(entry, dict) else {}


def sites_for(key):
    """Verified Feishu sites for a tenant; URLs without tenant_names stay explicit."""
    sites = []
    for item in _entry(key).get('sites') or []:
        if isinstance(item, str):
            sites.append({'url': item})
        elif isinstance(item, dict) and item.get('url'):
            sites.append(dict(item))
    return sites


def collect_platform(company, scope, output_dir):
    """Config-driven entrypoint matching p1_platform_beisen/moka's ``collect``."""
    ...
    if not sites:
        coverage = {
            ...
            'source_url': '', 'errors': ['no verified Feishu site configured for ' + key],
            'scope_evidence': 'Config-driven Feishu platform adapter; no verified site configured.',
            'scope_request': {'company': name, 'scope': scope, 'source_url': '',
                              'params': {'feishu_key': key, 'sites': []}},
            'evidence': [], 'evidence_files': [], 'pending_count': 0,
            'detail_missing_count': 0, 'source_missing_field_counts': {},
        }
        atomic(output_dir / 'result.json', {'jobs': [], 'coverage': coverage})
        atomic(output_dir / 'coverage.json', coverage)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--scope', choices=list(TYPES), default='campus')
    parser.add_argument('--output-dir', required=True, type=Path)
    args = parser.parse_args()
    payload = collect_platform(args.company, args.scope, args.output_dir)

...
def test_every_configured_line_is_a_company_and_disabled_lines_stay_out():
    # 20260919f surveyed 14 lines (6 enabled).  The 20260919g full-site batch
    # appended the other 54 public careersite tenants the census found to carry at
    # least one real posting, so the section is now 68 lines: 60 enabled careersite
    # tenants + the same 8 disabled survey lines (6 wxtemp + 德昌电机 + 太太乐).
    ...
    assert len(enabled) == 60 and len(disabled) == 8
...
def test_enabled_tenants_are_verified_careersite_sites_only():
    # The 6 originally verified tenants stay enabled; the 20260919g full-site batch
    # adds the other 54 public careersite tenants.  Every enabled line must be a
    # public careersite tenant: a wxtemp host must never be enabled.
    assert len(tupu.COMPANIES) == 60
    assert {'iqvia', 'lilly', 'schaeffler', 'bmw', 'innomotics', 'jnj'} <= set(tupu.COMPANIES)
    for key in tupu.COMPANIES:

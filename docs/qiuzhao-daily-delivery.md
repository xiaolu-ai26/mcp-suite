...
- 采集机上曾缺少 `normalize_tables.json`，证据只有 2026-09-19/20 的历史部署收据，本次没有实时核验，不写成当前已证实缺失。即使补齐这张表，`Morrisville` 这类不在词典里的城市仍会丢失。
...
- 国家未知时保持未知，不默认填中国；规范化写错的字段允许修正，不永久锁死。
...
- 先核实采集机上 `normalize_tables.json` 的实际状态；联想等公司映射、增量补全和全量投影使用同一个入口，结果一致。
